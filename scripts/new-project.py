...
def generate_claude_md(
    project_name: str, language: str, framework: str, commands: dict
) -> str:
...
def update_canonical(
    canonical: dict, repo_path: Path, language: str, framework: str, commands: dict
):
...
def create_github_labels(repo_path: Path):
    """Create standard GitHub labels for SpecKit workflow."""
    import subprocess
    ...
    if result.returncode != 0:
        print("Skipping GitHub labels (not a GitHub repo or gh not configured)")
        return

    labels = [
        ("auto-generated", "0E8A16", "Auto-generated by SpecKit"),
        ("priority-p1", "B60205", "Priority 1 - Critical"),
        ("priority-p2", "FBCA04", "Priority 2 - Normal"),
        ("priority-p3", "0E8A16", "Priority 3 - Low"),
        ("parallelizable", "1D76DB", "Can be worked on in parallel"),
        ("evolve", "5319E7", "Evolving requirement"),
    ]

    print("Creating GitHub labels for SpecKit workflow...")
    created = 0
...
def setup_project(repo_path: Path, language: str = None, framework: str = None):
    ...
    # Create GitHub labels for SpecKit workflow
    print()
    create_github_labels(repo_path)
...
def main():
    ...
    parser.add_argument(
        "language", nargs="?", help="Project language (python, typescript, etc.)"
    )
