...
Purpose: 3 of 4 agents (nautilus, utxoracle, n8n) have EMPTY workspaces — no SOUL.md, no instructions, no repo context. They behave generically. This plan gives each agent domain-specific personality, quality standards (TDD, anti-superficialità), and proper identity config.
...
<task type="auto">
  ...
  <action>
    ...
    2. Create SOUL.md — based on main's SOUL.md pattern but customized:
       ...
       - utxoracle: "You're Oracle, Sam's Bitcoin oracle engineer. Speciality: UTXOracle — exchange-free price oracle from on-chain UTXO data, pure Python, statistical clustering."
       - n8n: "You're Flow, Sam's workflow automation specialist. Speciality: N8N, Docker, Python, ML pipelines."
       - ALL agents share same quality standards section (TDD, anti-superficialità, prove concrete, security)
       - Keep each SOUL.md under 2KB (well within 20K bootstrapMaxChars limit)

    3. Create AGENTS.md — per-agent repo context:
       - nautilus: repos=nautilus_dev, LiquidationHeatmap, openBB_liquidity. Key: NautilusTrader framework, Cython adapters, backtesting engine, DuckDB analytics, market liquidity.
       - utxoracle: repos=UTXOracle only. Key: Bitcoin-native price oracle, pure Python, statistical clustering, on-chain data analysis.
    ...
    4. Create USER.md — SAME content for all agents (copy from main's USER.md):
       - Sam's preferences, communication style, security rules.

    5. Create IDENTITY.md — per-agent:
       - nautilus: name=Nautilus, emoji=🐙, theme="Algorithmic trading engineer"
       - utxoracle: name=Oracle, emoji=🔮, theme="Crypto data analyst"
       - n8n: name=Flow, emoji=⚡, theme="Workflow automation specialist"

    Important:
    - Use `docker exec openclaw-gateway sh -c 'cat > /path/file << "HEREDOC" ... HEREDOC'` pattern to write files
    - Do NOT duplicate main's AGENTS.md verbatim — each agent needs its own repo-specific content
...
<task type="auto">
  ...
  <action>
    ...
    1. Add `identity` blocks to each agent in `agents.list[]`:
       ```json
       {"id":"nautilus","workspace":"/home/node/clawd-nautilus","identity":{"name":"Nautilus","emoji":"🐙","theme":"Algorithmic trading engineer"}},
       {"id":"utxoracle","workspace":"/home/node/clawd-utxoracle","identity":{"name":"Oracle","emoji":"🔮","theme":"Crypto data analyst"}},
       {"id":"n8n","workspace":"/home/node/clawd-n8n","identity":{"name":"Flow","emoji":"⚡","theme":"Workflow automation specialist"}}
       ```
       Also add identity for main if missing: `{"name":"Bambam","emoji":"🦞","theme":"Senior dev, direct, competent"}`

    2. Add `thinkingDefault: "low"` to `agents.defaults` — low is good default, save tokens on cron/heartbeats. Users can `/think:medium` or `/think:high` for implementation sessions.

    3. Check existing cron entries — if any lack `--thinking off`, note them for manual update (cron config format may vary).
