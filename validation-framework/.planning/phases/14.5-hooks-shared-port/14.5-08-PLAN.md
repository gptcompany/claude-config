...
<task type="auto">
  <name>Task 1: Create hook-debugger.js library</name>
  <files>~/.claude/scripts/lib/hook-debugger.js</files>
  <action>
1. Create debug library with:
...
3. Automatic log rotation (max 1000 entries per hook)
  ...
  <done>hook-debugger.js provides full invocation tracking</done>
</task>
...
<task type="auto">
  <name>Task 4: Create hook-health.js (health monitoring)</name>
...
3. Status file: ~/.claude/debug/hooks/health.json
   ```json
   {
     "lastCheck": "ISO",
     "hooks": {
...
4. CLI command: `node hook-health.js --check` runs full health check
  </action>
  <verify>node ~/.claude/scripts/hooks/debug/hook-health.js --check</verify>
  <done>hook-health.js monitors hook health status</done>
</task>
