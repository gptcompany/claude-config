...
def get_current_version(
    package: str, pyproject_path: str = "pyproject.toml"
) -> Optional[str]:
...
def get_releases(owner: str, repo: str, token: Optional[str] = None) -> list:
...
def parse_version_safe(ver_str: str) -> Optional[version.Version]:
...
def check_breaking_changes(
    ...
    for release in releases:
        ...
        if not is_breaking:
            is_breaking = (
                "MAJOR VERSION" in text_to_check or "API CHANGE" in text_to_check
            )
...
Examples:
  ...
  %(prog)s --owner pydantic --repo pydantic --current 2.0.0 --output json
    ...
    parser.add_argument(
        "--owner", required=True, help="GitHub owner (e.g., nautechsystems)"
    )
    parser.add_argument(
        "--repo", required=True, help="GitHub repo (e.g., nautilus_trader)"
    )
    parser.add_argument(
        "--current", required=True, help="Current version (e.g., 1.200.0)"
    )
    parser.add_argument(
        "--target", help="Target version (optional, defaults to latest)"
    )
    parser.add_argument(
        "--token",
